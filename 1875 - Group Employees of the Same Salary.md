...
Use DENSE_RANK() to generate team_id.
Select the fields and order by team_id.
To make sure each team consists of at least two employees, use e1 and e2 from Employees such that they have different ids and the same salary.
```
<br>

```SQL
CREATE TABLE Employees (employee_id int, name varchar(32), salary int);

INSERT INTO Employees VALUES
(2, 'Meir', 3000), (3, 'Michael', 3000), (7, 'Addilyn', 7400),
(8, 'Juan', 6100), (9, 'Kannon', 7400);
...
SELECT *, DENSE_RANK() OVER (ORDER BY salary ASC) team_id
FROM Employees
WHERE salary IN (
    SELECT salary
    FROM Employees
