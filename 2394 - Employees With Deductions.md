...
All the times are in October 2022.

out_time can be one day after in_time which means the employee worked after the midnight.


In a company, each employee must work a certain number of hours every month.

Employees work in sessions.
...
Write a SQL query to report the IDs of the employees that will be deducted.
...
> Solutions Links
<br>
> 1. [here](https://github.com/kamyu104/LeetCode-Solutions/blob/master/MySQL/employees-with-deductions.sql)
<br>

```SQL
CREATE TABLE Employees (employee_id int, needed_hours int, PRIMARY KEY (employee_id));

CREATE TABLE Logs (employee_id int, in_time datetime, out_time datetime, PRIMARY KEY (employee_id, in_time, out_time));
