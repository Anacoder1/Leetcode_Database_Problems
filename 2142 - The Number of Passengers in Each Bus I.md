...
Write a SQL query to report the number of users that used each bus.
...
> Solution taken from [here](https://blog.51cto.com/u_10980859/5304981)
<br>

```SQL
CREATE TABLE Buses (bus_id int, arrival_time int, PRIMARY KEY (bus_id));
CREATE TABLE Passengers (passenger_id int, arrival_time int, PRIMARY KEY (passenger_id));
...
SELECT B.bus_id, COUNT(T.passenger_id) AS passengers_cnt
