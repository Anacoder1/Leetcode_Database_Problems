...
For each company_id, find the max salary and determine the tax rate, & calculate each employee’s salary after applying tax accordingly.
```
<br>
...
SELECT S.company_id, S.employee_id, S.employee_name, ROUND(S.salary * (1 - T.tax_rate)) AS salary
FROM Salaries AS S INNER JOIN (
    SELECT company_id, (
