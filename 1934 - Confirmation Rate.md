...
The confirmation rate of a user is calculated as the number of entries with 'confirmed' under the user divided by the total number of entries under the user.

Use IFNULL to deal with the case where there is no entry.

Use LEFT JOIN to keep the users that exist in Signups.
```
<br>
...
SELECT Signups.user_id, IFNULL(ROUND(SUM(action = 'confirmed') / COUNT(*), 2), 0.0) AS confirmation_rate
